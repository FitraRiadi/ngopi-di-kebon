...
class App {
  ...
  _fireClickAtX(clientX) {
    if (!this.medias || !this.medias.length) return;
    const rect = this.gl.canvas.getBoundingClientRect();
    // convert clientX to NDC x in [-1, 1]
    const ndcX = ((clientX - rect.left) / rect.width) * 2 - 1;
    // convert NDC to viewport space x (matching how OGL projects)
    const fov = (this.camera.fov * Math.PI) / 180;
    const vpHalfWidth = Math.tan(fov / 2) * this.camera.position.z * this.camera.aspect;
    const worldX = ndcX * vpHalfWidth;

    // find the media whose plane.position.x is closest to worldX
    let closestIdx = 0;
    ...
    // map back to original (non-duplicated) item index
    const originalLength = this.mediasImages.length / 2;
    const realIdx = closestIdx % originalLength;
